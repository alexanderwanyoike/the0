...
export const useCustomBotsSSE = (): UseCustomBotsSSEReturn => {
  ...
  // Handle SSE messages
  const handleSSEMessage = useCallback((data: CustomBotWithVersions[]) => {
  ...
  const handleSSEError = useCallback((error: Event) => {
    ...
    if (!initialLoadComplete) {
      ...
      console.log('SSE failed during initial load, falling back to manual fetch');
      fetchCustomBots();
    } else {
  ...
  const sseState = useSSEClient<CustomBotWithVersions[]>('/api/custom-bots/stream', {
  ...
  useEffect(() => {
    ...
    const fallbackTimeout = setTimeout(() => {
      if (!initialLoadComplete && !sseState.connected) {
        console.log('SSE connection taking too long, falling back to manual fetch');
        fetchCustomBots();
      }
    }, 5000);
