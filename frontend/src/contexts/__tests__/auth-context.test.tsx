...
function TestComponent() {
  const { user, loading, login, register, logout, token, authService } = useAuth();
  ...
  return (
    <div>
      ...
      <div data-testid="auth-service">{authService ? 'service available' : 'no service'}</div>
    </div>
  );
}
...
describe('AuthContext', () => {
  ...
  describe('initialization', () => {
    ...
    it('should register auth service with all utilities', async () => {
      ...
      const setAuthFetchService = require('@/lib/auth-fetch').setAuthFetchService;
      const setApiClientService = require('@/lib/api-client').setApiClientService;
  ...
  describe('auth service access', () => {
    it('should provide access to centralized auth service', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('auth-service')).toHaveTextContent('service available');
      });
    });
  });
  ...
  describe('backward compatibility', () => {
    it('should provide userData property for backward compatibility', async () => {
      ...
      function BackwardCompatTestComponent() {
        ...
        return <div data-testid="userdata">{userData?.username || 'no userData'}</div>;
      }
