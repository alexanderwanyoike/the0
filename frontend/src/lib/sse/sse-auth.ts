...
export const setSSEAuthService = (authService: any, logoutFn: () => Promise<void>) => {
 ...
 */
export function checkSSEPermission(endpoint: string): Result<{ hasPermission: true }, string> {
  try {
    if (!globalAuthService) {
      return {
        success: false,
