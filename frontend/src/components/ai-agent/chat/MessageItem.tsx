...
import { ArtifactIndicator } from "@/components/ai-agent/common/ArtifactIndicator";
...
export function MessageItem({ message, onContentUpdate }: MessageItemProps) {
  ...
  return (
    <div
      className={`w-full px-4 py-6 ${isUser ? "bg-background/50" : "bg-muted/30"}`}
    >
      <div className="max-w-4xl mx-auto flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="prose prose-neutral dark:prose-invert max-w-none">
            {message.isTyping ? (
              <TypingIndicator />
            ) : !isUser && (message.isStreaming || !message.isComplete) ? (
              <StreamingMessage
