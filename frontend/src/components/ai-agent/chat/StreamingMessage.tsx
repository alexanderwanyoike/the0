...
import { useStreamingTypewriter } from "@/hooks/ai-agent/useStreamingTypewriter";
import { useSettingsStore } from "@/stores/ai-agent/settingsStore";
import { Markdown } from "@/components/ai-agent/ui/markdown";
...
export function StreamingMessage({
  ...
  const { streamingEnabled, typewriterEnabled, typewriterSpeed } =
    useSettingsStore();
  ...
  useEffect(() => {
    if (isComplete && !isStreaming && isTyping) {
      console.log("🏁 Streaming complete, finishing typewriter");
      complete();
    }
  }, [isComplete, isStreaming, isTyping, complete]);
  ...
  const textToShow =
    streamingEnabled && typewriterEnabled && (isStreaming || isTyping)
      ? displayText
      : content;
