...
import { chatService, ChatSession } from "@/lib/ai-agent/chatService";
...
export function ChatList({ onSessionSelect, onNewChat }: ChatListProps) {
  ...
  // Remove duplicate session loading - useChatSessions hook already handles this
  ...
  const handleSaveTitle = async (sessionId: string) => {
    if (
      editTitle.trim() &&
      editTitle !== sessions.find((s) => s.id === sessionId)?.title
    ) {
      try {
