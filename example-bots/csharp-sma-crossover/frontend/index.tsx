 ...
 * - Displaying SMA values and crossover signals
 * - Visualizing price and moving average data
 *
 * The dashboard shows:
 * - Current price with SMA indicators
 * - Trading signals (BUY/SELL)
 * - Price and SMA history
 */
  ...
  return (
    <div className="p-4 space-y-6 font-mono">
      ...
      <div className="border-b border-green-900/50 pb-4">
        <h1 className="text-xl font-bold text-green-400">SMA Crossover Strategy</h1>
        <p className="text-gray-500 text-sm">
          {prices.length} price updates, {signals.length} signals generated
        </p>
      </div>

      {/* Current Price and SMA Card */}
          ...
          {signals
            ...
            .map((signal, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`border-l-4 ${isBuy ? "border-green-500 bg-green-950/20" : "border-red-500 bg-red-950/20"} p-3 rounded-r`}
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <span
                        className={`text-lg font-bold ${isBuy ? "text-green-400" : "text-red-400"}`}
                      >
                      ...
                      <div className="mt-2">
                        ...
                        <div className="w-24 h-1 bg-gray-700 rounded mt-1">
                          <div
                            className={`h-1 rounded ${isBuy ? "bg-green-500" : "bg-red-500"}`}
...
/**
 * Displays price history with SMA values
 */
function PriceHistory({ prices, smas }: { prices: BotEvent[]; smas: BotEvent[] }) {
  ...
  // Create a map of timestamps to SMA data
  const smaMap = new Map<string, { short: number; long: number }>();
  smas.forEach((sma) => {
    const data = sma.data as { short_sma: number; long_sma: number };
    smaMap.set(sma.timestamp.toISOString(), { short: data.short_sma, long: data.long_sma });
  });

  return (
    <div className="bg-gray-900 border border-gray-800 rounded p-4">
      <h2 className="text-sm font-semibold text-gray-400 mb-3">
        Price & SMA History (Last {Math.min(prices.length, 20)})
      </h2>
