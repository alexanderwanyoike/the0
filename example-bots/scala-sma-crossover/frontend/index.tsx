  ...
  return (
    <div className="p-4 space-y-6 font-mono">
      <div className="border-b border-gray-200 dark:border-gray-800 pb-4">
        <h1 className="text-xl font-bold text-green-600 dark:text-green-400">SMA Crossover Strategy (Scala 3)</h1>
    ...
    <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded p-6">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <span className="text-gray-600 dark:text-gray-400">SMA {data.short_period} (Fast)</span>
          <span className="text-xl font-bold text-yellow-600 dark:text-yellow-400">${data.short_sma.toLocaleString()}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-gray-600 dark:text-gray-400">SMA {data.long_period} (Slow)</span>
          <span className="text-xl font-bold text-blue-600 dark:text-blue-400">${data.long_sma.toLocaleString()}</span>
        </div>
      </div>
      <div className={`mt-4 p-2 rounded text-center ${isBullish ? "bg-green-100 dark:bg-green-950/30 border border-green-300 dark:border-green-800" : "bg-red-100 dark:bg-red-950/30 border border-red-300 dark:border-red-800"}`}>
        <span className={`text-sm font-medium ${isBullish ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}>
      ...
      {signals.length === 0 ? (
        ...
        <div className="space-y-3">
          {signals.slice(-5).reverse().map((signal, i) => {
            ...
            return (
              <div key={i} className={`border-l-4 ${isBuy ? "border-green-500 bg-green-50 dark:bg-green-950/20" : "border-red-500 bg-red-50 dark:bg-red-950/20"} p-3 rounded-r`}>
                <div className="flex justify-between items-start">
                  <div>
                    <span className={`text-lg font-bold ${isBuy ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}>
                      {data.type} {data.symbol}
                    </span>
                    <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">@ ${data.price.toLocaleString()}</p>
