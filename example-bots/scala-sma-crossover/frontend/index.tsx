 ...
 * =======================
 * Custom frontend for the Scala 3 SMA crossover bot.
 */
  ...
  return (
    <div className="p-4 space-y-6 font-mono">
      <div className="border-b border-border pb-4">
        <h1 className="text-xl font-bold text-primary">SMA Crossover Strategy (Scala 3)</h1>
  ...
  return (
    <div className="bg-muted/50 border border-border rounded p-6">
      ...
      <div className="flex items-baseline gap-4 mb-2">
        <span className="text-4xl font-bold text-primary">${data.value.toLocaleString()}</span>
...
function SMACard({ sma }: { sma: BotEvent | null }) {
  ...
  return (
    <div className="bg-muted/50 border border-border rounded p-6">
      <div className="text-sm text-muted-foreground mb-4">Moving Averages</div>
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground">SMA {data.short_period} (Fast)</span>
          <span className="text-xl font-bold text-yellow-600">${data.short_sma.toLocaleString()}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground">SMA {data.long_period} (Slow)</span>
          <span className="text-xl font-bold text-blue-600">${data.long_sma.toLocaleString()}</span>
        </div>
      </div>
      <div className={`mt-4 p-2 rounded text-center ${isBullish ? "bg-green-500/20 border border-green-600/30" : "bg-red-500/20 border border-red-600/30"}`}>
...
function SignalsList({ signals }: { signals: BotEvent[] }) {
  return (
    <div className="bg-muted/50 border border-border rounded p-4">
      <h2 className="text-sm font-semibold text-foreground mb-3">Trading Signals ({signals.length})</h2>
      {signals.length === 0 ? (
        <p className="text-muted-foreground text-sm">No signals yet - waiting for SMA crossover</p>
      ) : (
        <div className="space-y-3">
          {signals.slice(-5).reverse().map((signal, i) => {
            ...
            return (
              <div key={i} className={`border-l-4 ${isBuy ? "border-green-500 bg-green-500/10" : "border-red-500 bg-red-500/10"} p-3 rounded-r`}>
                <div className="flex justify-between items-start">
                  <div>
                    <span className={`text-lg font-bold ${isBuy ? "text-green-600" : "text-red-600"}`}>
                      {data.type} {data.symbol}
                    </span>
                    <p className="text-muted-foreground text-sm mt-1">@ ${data.price.toLocaleString()}</p>
                    <p className="text-muted-foreground text-sm mt-1">{data.reason}</p>
                    <div className="mt-2">
...
function PriceHistory({ prices, smas }: { prices: BotEvent[]; smas: BotEvent[] }) {
  ...
  return (
    <div className="bg-muted/50 border border-border rounded p-4">
      <h2 className="text-sm font-semibold text-foreground mb-3">Price & SMA History (Last {Math.min(prices.length, 20)})</h2>
            ...
            {prices.slice(-20).reverse().map((event, i) => {
              ...
              return (
                <tr key={i} className="border-b border-border/50">
                  ...
                  <td className={`py-2 text-right ${isPositive ? "text-green-600" : "text-red-600"}`}>
                    {isPositive ? "+" : ""}{data.change_pct.toFixed(3)}%
                  </td>
