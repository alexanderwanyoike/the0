...
describe('FxMacroDataClient', () => {
  it('builds authenticated read endpoint URLs', async () => {
    const seen: string[] = [];
    ...
    expect(url.pathname).toBe('/api/v1/calendar/usd');
    expect(url.searchParams.get('start_date')).toBe('2026-07-01');
    expect(url.searchParams.get('end_date')).toBe('2026-07-31');
    expect(url.searchParams.get('api_key')).toBe('test-key');
  });

  it('exposes helpers for the read API surface', async () => {
    const paths: string[] = [];
    const client = new FxMacroDataClient({
