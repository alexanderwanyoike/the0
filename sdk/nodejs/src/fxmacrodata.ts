export type FxMacroDataQueryValue =
  | string
  | number
  | boolean
  | Date
...
export type FxMacroDataQuery = Record<
  string,
  FxMacroDataQueryValue | FxMacroDataQueryValue[]
>;
...
export class FxMacroDataClient {
  ...
  async request<T = unknown>(
    ...
    if (!response.ok) {
      ...
      throw new Error(`FXMacroData request failed: ${response.status} ${text}`);
    }
  ...
  async graphql<T = unknown>(request: GraphQLRequest): Promise<T> {
