...
public static class Input
{
    ...
    /// All output functions use this prefix so you can freely use Console.WriteLine for logging.
    /// </summary>
    private const string ResultMarker = "THE0_RESULT:";
    /// <summary>
    ...
    /// Uses the THE0_RESULT: marker so you can freely use Console.WriteLine for logging.
    /// </summary>
    /// <param name="message">The success message to include in the output</param>
    public static void Success(string message)
    {
        var escaped = message.Replace("\\", "\\\\").Replace("\"", "\\\"");
        Console.WriteLine($"{ResultMarker}{{\"status\":\"success\",\"message\":\"{escaped}\"}}");
    }
    ...
    {
        var escaped = message.Replace("\\", "\\\\").Replace("\"", "\\\"");
        Console.WriteLine($"{ResultMarker}{{\"status\":\"error\",\"message\":\"{escaped}\"}}");
        Environment.Exit(1);
    }
    ...
    {
        Console.WriteLine($"{ResultMarker}{JsonSerializer.Serialize(data)}");
    }
}
