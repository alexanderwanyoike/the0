...
import { generateArtifactZip, generateExportFilename } from '@/utils/zipGenerator'
...
declare global {
  interface Window {
    pywebview?: {
      api?: {
        save_file?: (content: string, filename: string, filters?: string) => Promise<string>
        save_file_dialog?: (content: string, filename: string) => Promise<string>
 ...
 */
async function exportAsZip(files: ArtifactFile[], filename: string): Promise<ExportResult> {
 ...
 */
async function exportAsJson(files: ArtifactFile[], filename: string): Promise<ExportResult> {
  const exportData = {
 ...
 */
async function saveFileInPywebview(blob: Blob, filename: string): Promise<ExportResult> {
  try {
    ...
    console.error('PyWebview save failed, falling back to browser download:', error)
    return saveBlobInBrowser(blob, filename)
  }
}
...
export function getExportSizeInfo(files: ArtifactFile[]): {
  ...
  const totalSize = fileFiles.reduce((total, file) => total + (file.content?.length || 0), 0)
