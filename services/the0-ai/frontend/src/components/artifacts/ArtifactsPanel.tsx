...
export function ArtifactsPanel() {
  ...
  if (error) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg mb-2 text-destructive">Error loading artifacts</p>
  ...
  return (
    <div className="h-full flex bg-background">
      <div className="w-80 border-r border-border">
        {files.length > 0 ? (
          <FileTree files={files} selectedFile={activeFile} onFileSelect={setActiveFile} />
        ) : (
          <div className="p-4 text-center text-muted-foreground">
            <p className="text-sm">Start chatting with the0 to create trading bot files</p>
          </div>
        )}
      </div>
