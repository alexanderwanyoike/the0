...
describe('artifactsStore', () => {
  ...
  it('should clear active file when deleted file is active', () => {
    const { setFiles, setActiveFile, deleteFile } = useArtifactsStore.getState()
  ...
  it('should manage selected files correctly', () => {
    const { addSelectedFile, removeSelectedFile, clearSelectedFiles } = useArtifactsStore.getState()
