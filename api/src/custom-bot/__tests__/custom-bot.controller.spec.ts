...
describe("CustomBotController", () => {
  ...
  describe("createCustomBot", () => {
    ...
    it("should throw BadRequestException when user ID is missing", async () => {
      const userWithoutUid = {} as any;
      const filePath =
        "user123/test-bot/1.0.0/test-bot_1.0.0_123456.zip";
      const body = { config: JSON.stringify(validConfig), filePath };

      await expect(
        controller.createCustomBot("test-bot", body, userWithoutUid),
  ...
  describe("updateCustomBot", () => {
    ...
    it("should throw BadRequestException when user ID is missing", async () => {
      ...
      await expect(
        controller.updateCustomBot("test-bot", body, userWithoutUid),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("createCustomBot - filePath scoping", () => {
    it("should reject filePath not scoped to the requesting user", async () => {
      const filePath = "other-user/test-bot/1.0.0/test-bot_1.0.0_123456.zip";
      const body = { config: JSON.stringify(validConfig), filePath };
    ...
    it("should allow filePath scoped to the requesting user", async () => {
  ...
  describe("updateCustomBot - filePath scoping", () => {
    it("should reject filePath not scoped to the requesting user", async () => {
      const filePath = "other-user/test-bot/1.0.0/test-bot_1.0.0_123456.zip";
      const body = { config: JSON.stringify(validConfig), filePath };
  ...
  describe("getUserCustomBots", () => {
    ...
    it("should throw BadRequestException when user ID is missing", async () => {
      ...
      await expect(
        controller.getUserCustomBots(userWithoutUid),
      ).rejects.toThrow(BadRequestException);
        ...
        versions: [
          {
            version: "1.1.0",
            config: validConfig,
            filePath: "user123/test-bot/1.1.0/v1.1.zip",
            createdAt: new Date(),
            updatedAt: new Date(),
          },
          {
            version: "1.0.0",
            config: validConfig,
            filePath: "user123/test-bot/1.0.0/v1.0.zip",
            createdAt: new Date(),
            updatedAt: new Date(),
          },
        ],
