...
func (m *minIOLogger) AppendBotLogs(ctx context.Context, id string, logs string) error {
	...
	dailyKey := fmt.Sprintf("logs/%s/%s.log", id, timestamp)
	...
	tmpKey := fmt.Sprintf("logs/%s/.tmp-%d", id, time.Now().UnixNano())
	...
	// Always clean up the temp object when we are done.
	defer func() {
		_ = m.client.RemoveObject(ctx, m.logsBucket, tmpKey, minio.RemoveObjectOptions{})
	}()
